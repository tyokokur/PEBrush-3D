...
void DGC_STEP(void){
	int X;
	double t1=0,t2=0,t3=0,t4=0,t5=0,t6=0,t7=0,cutime=0;
	for (X=0; X<NF_N; X++) {
		
		//timer_start(&t1, &cutime);
		DGC_STEP_INIT(X);
		//timer_stop(&t1, &cutime);

		//timer_start(&t2, &cutime);
		qInt_Free();
		//timer_stop(&t2, &cutime);

		//timer_start(&t3, &cutime);
		DGC_FFT_BACKWARD(X);
		//timer_stop(&t3, &cutime);

		//timer_start(&t4, &cutime);
		qInt_Graft(X, qcA); 
		//timer_stop(&t4, &cutime);

		//timer_start(&t5, &cutime);
		DGC_FFT_FORWARD(X);
		//timer_stop(&t5, &cutime);

		//timer_start(&t6, &cutime);
		get_PHI();
		//timer_stop(&t6, &cutime);

		//timer_start(&t7, &cutime);
		get_EPS_PROF();
		//timer_stop(&t7, &cutime);
		...
		/*
	printf("cutime: %.2f\n", cutime);
	printf("DGC STEP INIT  : %.2E (%.2f\%)\n", t1, t1/cutime*100);
	printf("QINT_FREE      : %.2E (%.2f\%)\n", t2, t2/cutime*100);
	printf("BACKWARD       : %.2E (%.2f\%)\n", t3, t3/cutime*100);
	printf("QINT GRAFT     : %.2E (%.2f\%)\n", t4, t4/cutime*100);
	printf("FORWARD        : %.2E (%.2f\%)\n", t5, t5/cutime*100);
	printf("GET PHI        : %.2E (%.2f\%)\n", t6, t6/cutime*100);
	printf("GET EPS PROF   : %.2E (%.2f\%)\n", t7, t7/cutime*100);
		*/
...
void DGC_FFT_BACKWARD(int X) {
	...
	//timer_stop(&t2,&cut);
	//printf("Backward DGC_COS: %.5E\n", t1);
	//printf("Backward QcA    : %.5E\n", t2);
}
...
void DGC_SETUP(void) {
	...
	// PHI integral cutoff
	if (DGC_lzmax < lz) DGC_Nzmax = roundl(DGC_lzmax / dz);
	else DGC_Nzmax = Nz;
...
void DGC_CLEAN(void){
	...
	free(deltaFun);
	//free(GAUSSPROP); 
	free(xGAUSSDIST); free(yGAUSSDIST); free(zGAUSSDIST);
	free(zGAUSSPROP);
}
