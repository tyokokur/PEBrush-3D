...
void FFTW_SETUP(void){
	...
	fftw_plan_with_nthreads(1); //TODO
	*/
...
double tic = omp_get_wtime();
		...
		fftw_inc_xy    = (double *) fftw_malloc(NxNy*sizeof(double));
		fftw_outc_r_xy = (double *) fftw_malloc(NxNy*sizeof(double));
		cfp_xy = fftw_plan_r2r_2d(Nx, Ny, fftw_inc_xy, fftw_outc_r_xy, FFTW_REDFT00, FFTW_REDFT00, FFTW_PATIENT); 
		cip_xy = fftw_plan_r2r_2d(Nx, Ny, fftw_outc_r_xy, fftw_inc_xy, FFTW_REDFT00, FFTW_REDFT00, FFTW_PATIENT);
		...
		// FFT full
		fftw_inc    = (double *) fftw_malloc(NxNyNz*sizeof(double));
		fftw_outc_r = (double *) fftw_malloc(NxNyNz*sizeof(double));
		cfp = fftw_plan_r2r_3d(Nx, Ny, Nz, fftw_inc, fftw_outc_r, FFTW_REDFT00, FFTW_REDFT00, FFTW_REDFT00, FFTW_MEASURE);
		cip = fftw_plan_r2r_3d(Nx, Ny, Nz, fftw_outc_r, fftw_inc, FFTW_REDFT00, FFTW_REDFT00, FFTW_REDFT00, FFTW_MEASURE);
		//OLD: cfp = fftw_plan_r2r_3d(Nx, Ny, Nz, fftw_inc, fftw_outc_r, FFTW_REDFT10, FFTW_REDFT10, FFTW_REDFT10, FFTW_PATIENT);
		//OLD: cip = fftw_plan_r2r_3d(Nx, Ny, Nz, fftw_outc_r, fftw_inc, FFTW_REDFT01, FFTW_REDFT01, FFTW_REDFT01, FFTW_PATIENT);

		// DGC 
		zGAUSSPROP = INIT(Nz); 
		//FG_in     = INIT(NxNy);
		FG_out    = (double *) fftw_malloc(NxNy*sizeof(double));
		GAUSSPROP = (double *) fftw_malloc(NxNy*sizeof(double));
		//OLD: cfp_FG = fftw_plan_r2r_2d(Nx, Ny, GAUSSPROP, FG_out, FFTW_REDFT10, FFTW_REDFT10, FFTW_PATIENT);
		cfp_FG = fftw_plan_r2r_2d(Nx, Ny, GAUSSPROP, FG_out, FFTW_REDFT00, FFTW_REDFT00, FFTW_MEASURE);
...
void DGC_COS(double *init, double **g, int K, double *W, int *Ns, int sign, double *fftw_in, double *fftw_out, double *GAUSSPROP, double *FG_in, double *FG_out){
	...
	get_2D_GAUSSPROP(0, GAUSSPROP, xGAUSSDIST, Nx, yGAUSSDIST, Ny); 
	fftw_execute(cfp_FG); // GAUSSPROP -> FG_out

	double **fftw_k; fftw_k = (double **) fftw_malloc(Nz*sizeof(double)); 
	for (s=0;s<Nz;s++) fftw_k[s] = (double *) fftw_malloc(NxNy*sizeof(double));

#pragma omp parallel num_threads(OMP_NUM_THREADS)\
       firstprivate(zGAUSSDIST, FG_out, cfp_xy, cip_xy) shared(g, fftw_k)
{//Threads traverse is, k together
	int is, s;
	double *FG_out_perm; FG_out_perm = INIT(NxNy);
	memcpy(FG_out_perm, FG_out, NxNy*sizeof(double));
	...
	for (is = 1; is <= Nsf; is++){
		...
		int k, ij, nt; 
		for (k=0; k<Nz; k++){
			#pragma omp for schedule(static,2*Nzg_2)
			for (ij=0; ij<NxNy; ij++){
				int kp,fsum,bsum;
				double a,b;
				for (a=0.0, kp=k+1, fsum=0; kp<Nz && fsum<Nzg_2; kp++, fsum++){
					a += zGAUSSDIST[fsum+1]*g[ij*Nz+kp][s-sign];
				}
				
				for (b=0.0, kp=k-1, bsum=0; kp>=0 && bsum<Nzg_2; kp--, bsum++){
					b += zGAUSSDIST[bsum+1]*g[ij*Nz+kp][s-sign];
				}

				fftw_k[k][ij] = dz*(zGAUSSDIST[0]*g[ij*Nz+k][s-sign] + a + b);
			} 
		}

		#pragma omp for
		for (k=0; k<Nz; k++){
			double *fftw_in = fftw_malloc(NxNy*sizeof(double));
			double *fftw_out = fftw_malloc(NxNy*sizeof(double));

			memcpy(fftw_in, fftw_k[k], NxNy*sizeof(double));
			fftw_execute_r2r(cfp_xy, fftw_in, fftw_out); // fftw_in --> fftw_out
		
			for (ij=0; ij<NxNy; ij++) fftw_out[ij] = fftw_out[ij]*FG_out_perm[ij];

			fftw_execute_r2r(cip_xy, fftw_out, fftw_in); // fftw_out --> fftw_in

			for (ij=0;ij<NxNy;ij++) g[ij*Nz+k][s] = exp(-W[ij*Nz+k])*fftw_in[ij]/(4.0*(Nx-1)*(Ny-1))/v0;
	...
	for (s=0;s<Nz;s++) fftw_free(fftw_k[s]);
	fftw_free(fftw_k);
}
...
//printf("%d ", iter_PB);
		...
		memcpy(pot_elec_old, pot_elec, NxNyNz*sizeof(double));
		...
		for (ijk=0; ijk<NxNyNz; ijk++) pot_elec[ijk] = fftw_in[ijk] / (8.0*(Nx-1)*(Ny-1)*(Nz-1)); 
		//OLD: for (ijk=0; ijk<NxNyNz; ijk++) pot_elec[ijk] = fftw_in[ijk] / (8.0*NxNyNz); 
...
void FFTW_CLEAN(void) {
	...
	if (HALF_BOOL){
		fftw_free(fftw_inc);
		fftw_free(fftw_inc_xy);
		fftw_free(fftw_outc_r_xy);
		fftw_free(fftw_outc_r);
		fftw_free(GAUSSPROP);
		fftw_free(FG_out);
	}
