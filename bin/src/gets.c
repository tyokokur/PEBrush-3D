...
//printf("\t[gets.c/get_freeEnergy] (%d threads): %f\n", nthreads, toc-tic);
	...
	freeEnergy=freeU+freeW+freeS+free_elec; //+free_factorial; 
...
double tic = omp_get_wtime();
	for (Y=0; Y<K_i[X]; Y++) {
		...
		#pragma omp parallel \
			shared(nf, ni, Y)\
			private(ijk, s, sum1, sum2, ij, k)
		{ 
		#pragma omp for 
		for (k=0; k<DGC_Nzmax; k++){
			for (ij=0; ij<NxNy; ij++){
			ijk = ij*Nz + k;
		//for (ijk=0; ijk<NxNyNz; ijk++) {
			sum2 = 0.0;
...
//Anderson Mixing
	...
	for (n=1; n<=and_NrMax; n++){ 
		for (X=0; X<NF_N; X++){ 
			for (Y=0;Y<K_i[X];Y++){
				memcpy(DAs[n-1][X], DAs[n][X], K_i[X]*NxNyNz*sizeof(double));
				memcpy(DBs[n-1], DBs[n], NxNyNz*sizeof(double));
				memcpy(WAs[n-1][X], WAs[n][X], K_i[X]*NxNyNz*sizeof(double));
				memcpy(WBs[n-1], WBs[n], NxNyNz*sizeof(double));
				/*	
				#pragma omp parallel for \
					shared(n, X, Y) \
					private(ijk)
	...
	for (ijk=0;ijk<NxNyNz;ijk++){
		...
		for (X=0;X<NF_N;X++) for (Y=0;Y<K_i[X];Y++) {
			...
			DAs[end][X][_Yijk] = WAdiff[X][_Yijk] - wcmp*psum; //Update last val
			WAs[end][X][_Yijk] = WA[X][_Yijk]; //Update last val
			#pragma omp atomic
			and_num += WAdiff[X][_Yijk]*WAdiff[X][_Yijk];
			and_den += WA[X][_Yijk]*WA[X][_Yijk];
		}
		wBnew[ijk] = 0;
		for (X=0;X<NF_N;X++) for (Y=0;Y<K_i[X];Y++) wBnew[ijk] += Chi_i[X][Y]*PHA[X][_Yijk];
		wBnew[ijk] -= eta[ijk];
		wBdiff[ijk] = wBnew[ijk]-wB[ijk];		
		DBs[end][ijk] = wBdiff[ijk] - wcmp*psum; //Update last val 
		WBs[end][ijk] = wB[ijk]; //Update last val
		#pragma omp atomic
		and_num += wBdiff[ijk]*wBdiff[ijk];
		and_den += wB[ijk]*wB[ijk];
	}	
	and_err = and_num/and_den;
	and_err = pow(and_err, 0.50);
	
	if (iter%and_it!=0 || and_NrMax == 0 || and_rep_N == 0){// || and_err>1e-02 || and_den>1e-03 || and_num>1e-3){ 
		for (ijk=0; ijk<NxNyNz; ijk++){
			psum = 1.0-phA[ijk]-phB[ijk];
	...
	//if (iter%10==0) printf("err: %.5e, and_num: %.3e, and_den: %.3e\n", and_err, and_num, and_den);
	
	if (and_rep_N > 0) and_rep_N -= 1;
	and_Nr = fmin(iter-1, and_NrMax); 
	//and_Nr = fmin(iter, and_NrMax); 
...
printf("\n");
	*/
	...
	//Half step
	for (X=0;X<NF_N;X++) for (Y=0;Y<K_i[X];Y++){ 
		//WAnew[X][_Yijk] = WAs[end][X][_Yijk]; 
		//DAnew[X][_Yijk] = DAs[end][X][_Yijk];
		memcpy(WAnew[X], WAs[end][X], K_i[X]*NxNyNz*sizeof(double)); //k+1/2
		memcpy(DAnew[X], DAs[end][X], K_i[X]*NxNyNz*sizeof(double));
		for (ijk=0;ijk<NxNyNz;ijk++) for (n=1;n<=and_Nr;n++) {
			WAnew[X][_Yijk] += Cs[n-1]*(WAs[end-n][X][_Yijk]-WAs[end][X][_Yijk]);
			DAnew[X][_Yijk] += Cs[n-1]*(DAs[end-n][X][_Yijk]-DAs[end][X][_Yijk]);
		}
	}
	memcpy(wBnew, WBs[end], NxNyNz*sizeof(double));
	memcpy(DBnew, DBs[end], NxNyNz*sizeof(double));
	for (ijk=0;ijk<NxNyNz;ijk++){
		//wBnew[ijk] = WBs[end][ijk]; //k+1/2
		//DBnew[ijk] = DBs[end][ijk];
		for (n=1; n<=and_Nr; n++){
			wBnew[ijk] += Cs[n-1]*(WBs[end-n][ijk] - WBs[end][ijk]);
			DBnew[ijk] += Cs[n-1]*(DBs[end-n][ijk] - DBs[end][ijk]);
		}
	}	
	//Update fields
	lambda = 1.0-pow(0.9, iter);
	...
	for (ijk=0;ijk<NxNyNz;ijk++){
		...
		for (X=0;X<NF_N;X++) for (Y=0;Y<K_i[X];Y++){
			...
			WA[X][_Yijk] = (1.0-wand)*WA[X][_Yijk] + wand*(WAnew[X][_Yijk] + lambda * DAnew[X][_Yijk] - wcmp*psum);
		}
		//wB[ijk] = wBnew[ijk] + lambda * DBnew[ijk];
		wB[ijk] = (1.0-wand)*wB[ijk] + wand*(wBnew[ijk] + lambda * DBnew[ijk] - wcmp*psum);
	}
