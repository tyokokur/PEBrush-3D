...
void SETUP(void){
	stdout_redir_setup();
	stdout_redir_start();
	FILE *fp = fopen("times.txt","w"); fclose(fp); // clear
	LOAD_AA(seq_file);
	ASSIGN_CONSTS();
	REPORT_SETUP();
	CALLOC_FIELDS();
...
void INIT_PHA_WA_SPREAD(char *Win, int win_box) {
	...
	double **WA1; WA1 = INIT_2D(WA1, NF_N, K_i[i]*win_nx*win_ny*win_nz);
	double *wB1; wB1 = INIT(win_nx*win_ny*win_nz);
	double *eta1; eta1 = INIT(win_nx*win_ny*win_nz);
	double *pot_elec1; pot_elec1 = INIT(win_nx*win_ny*win_nz);
	fp=fopen(Win,"r"); 
	for (ijk=0; ijk<win_nx*win_ny*win_nz; ijk++){
		for (X=0; X<NF_N; X++) {
			fscanf(fp, "[ ");
			for (Y=0; Y<K_i[X]-1; Y++) fscanf(fp, "%lf ", &WA1[X][_Yijk]);
			fscanf(fp, "%lf ] ", &WA1[X][_Yijk]);
		}
		fscanf(fp, "%lf %lf %lf\n", &wB1[ijk], &eta1[ijk], &pot_elec1[ijk]);
	}
	fclose(fp);
	...
	for (k=0; k<Nz; k++) {
		if (k >= win_nz){
			...
			for (i=0;i<Nx;i++) for (j=0;j<Ny;j++) {
				if (i < win_nx && j < win_ny) { //transfer original
					ijk1 =(i*win_ny + j)*win_nz + k;
				}
				else if (i >= win_nx && j >= win_ny) { //transfer corner
					ijk1 = ((win_nx-1)*win_ny + (win_ny-1))*win_nz + k;
				}
				else if (i >= win_nx) { //transfer edge
					ijk1 =((win_nx-1)*win_ny + j)*win_nz + k;
				}
				else if (j >= win_ny) { //transfer edge
					ijk1 =(i*win_ny + (win_ny-1))*win_nz + k;
				}
				ijk = (i*Ny + j)*Nz + k;
				wB[ijk]  = wB1[ijk1];
				eta[ijk] = eta1[ijk1];
				pot_elec[ijk] = pot_elec1[ijk1];
				for (X=0; X<NF_N; X++) for (Y=0; Y<K_i[X]; Y++) WA[X][_Yijk] = WA1[X][Y*win_nx*win_ny*win_nz+ijk1];
			}
		}
	}
	...
	for (i=0;i<NF_N;i++) free(WA1[i]); free(WA1);
	free(eta1); free(wB1); free(pot_elec1);
}
